...
- **Easter eggs** (gandalf.gif overlay/bubble + the intro airplane/explosion animation + the `master.png` GPS marker) are **off by default** and toggled by **double-tapping the `#brandTitle` ("Quota_Zero") pill** (manual two-`click` counter within 500 ms via `toggleEggs()` — not `dblclick`, which is unreliable on mobile; the element has `touch-action: manipulation` to kill the double-tap-zoom gesture). When on, the title turns danger-pink (`.eggs-on`) — no toast/message. State persists in localStorage `quotazero_eggs_v1` (`"1"` = on). The live `eggsEnabled` flag short-circuits `runIntroAnimation` (intro plays only on a load while enabled), `updateGandalf`, and `maybeTriggerGandalf`; the dblclick handler re-runs `updateGandalf()` and `setUserMarker()` so everything switches immediately. The intro: planes converge on a target, explosion fires at `EXPLOSION_LEAD_T` (0.88) and is drawn `EXPLOSION_Y_OFFSET` (-20px) higher, planes are removed at `PLANE_END_T` (0.95), and a `master.png` sprite spawns from the explosion centre (below it in DOM order), sliding south-east at plane speed until off-screen; `maybeCleanup()` removes the layer only once both the explosion is done and the master has left the viewport.
- **GPS position marker** (`setUserMarker`, both non-interactive `L.marker`s so they never block clicks): default = classic teal pulsing dot (`.gps-marker` divIcon). When eggs are on = `master.png` image marker (`.gps-master`, 26×69 px, bottom-anchored) with a recurring **speech bubble** (`.gps-bubble` Leaflet tooltip) that alternates "Eccomi!" / "Sono qui!" — visible 2 s, hidden 10 s (`showGpsBubble`/`hideGpsBubble`/`startGpsBubble`/`stopGpsBubble`).
