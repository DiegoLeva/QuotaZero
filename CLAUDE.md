...
- **Gestione PF** section (bottom of the left sidebar): bulk ops on OUR added PFs. `exportRilievoGeojson()` GETs all `/api/punti-rilievo` (no bbox) → downloads a GeoJSON FeatureCollection (Point geom, props: id/comune/seq/nome/createdAt; **no photos**). `importRilievoGeojson(file)` parses a GeoJSON and POSTs each `Point` feature as a new PF (server reassigns `seq`). `deleteAllRilievo()` fetches all then DELETEs them one by one (no bulk-delete endpoint), behind a `confirm()`.
