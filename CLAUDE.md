...
| Isoipse Lazio (curve di livello 5 m) | `https://geoportale.regione.lazio.it/geoserver/ows` — layer `geonode:curve_livello`. The Lazio GeoServer **refuses inline `SLD_BODY`** (returns `Dynamic style usage is forbidden`) and its single published style draws unlabeled orange lines, so we cannot label the contours via WMS. The frontend instead fetches the layer as **WFS 1.0.0 GeoJSON** (`outputFormat=application/json`, `srsName=EPSG:4326`, bbox-filtered, `maxFeatures=600`) on every `moveend`/`zoomend` (debounced 320 ms), renders each polyline with `L.geoJSON` in yellow `#ffcc33` on an SVG renderer pinned to `isoipsePane` (z-index 475), then uses **`leaflet-textpath`** (loaded from CDN) to write the `cv_liv_q` value repeated along each curve with a dark halo. Hidden below zoom 13 to avoid pulling thousands of features. |
