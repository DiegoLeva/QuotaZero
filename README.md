...
- **Personalizza colore e opacità**: dal pannello laterale si apre un dialog dove per ciascun livello (isoipse, particelle, fabbricati, numeri, sfondo, punti fiduciali) si possono regolare colore e opacità con anteprima immediata. Per le isoipse c'è anche la checkbox per accendere/spegnere i valori di quota. Le scelte si salvano in `localStorage` (chiave `quotazero_prefs_v1`) insieme allo stato on/off di ogni livello, allo sfondo selezionato e all'opacità globale, e vengono ripristinate a ogni sessione.
- **Indicatore di zoom** sopra ai pulsanti +/− della mappa: badge cyan con il numero intero del livello di zoom corrente.
